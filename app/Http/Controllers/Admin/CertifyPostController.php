...
use App\Http\Requests\Admin\CertifyPostRequest;
use App\Models\CertifyPost;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\View;
use Yajra\Datatables\Datatables;

class CertifyPostController extends Controller
{
    public function __construct(CertifyPost $model)
    {        
        $this->moduleName = "CertifyPost";
        $this->moduleRoute = url('admin/certifypost');
        $this->moduleView = "admin.main.certifypost";
        $this->model = $model;
    ...
    public function store(CertifyPostRequest $request)
    {
        $input = $request->except(['_token', 'image']);
        try {         
            if ($request->file('image', false)) {        
                $imageStore = GreenPlacesHelpers::saveUploadedImage($request->file('image'), config("greenplaces.path.doc.certifypost_image"), "", $isCreateThumb="1", $height=null, $width=700, $request->get('cropped_image'));            
    ...
    public function update(CertifyPostRequest $request, $id)
    {                         
        $input = $request->except(['_token', 'image']);           
        try {
            ...
            if ($result) {                                          
                if ($request->file('image', false)) {        
                    $imageStore = GreenPlacesHelpers::saveUploadedImage($request->file('image'), config("greenplaces.path.doc.certifyPost_image"), $result->image, $isCreateThumb="1", $height=null, $width=700, $request->get('cropped_image'));            
                    if (isset($imageStore['error_msg']) && $imageStore['error_msg'] == '' && isset($imageStore['name']) && !empty($imageStore['name'])) {
                        $input["image"] = $imageStore['name'];                    
                    }                                
                } 
