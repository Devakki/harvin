...
use App\Http\Requests\Admin\CredibilityBrandRequest;
use App\Models\CredibilityBrand;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\View;
use Yajra\Datatables\Datatables;

class CredibilityBrandController extends Controller
{
    public function __construct(CredibilityBrand $model)
    {        
        $this->moduleName = "CredibilityBrand";
        $this->moduleRoute = url('admin/credibilitybrands');
        $this->moduleView = "admin.main.credibilitybrands";
        $this->model = $model;
    ...
    public function store(CredibilityBrandRequest $request)
    {
        $input = $request->except(['_token', 'image']);
        try {         
            if ($request->file('image', false)) {        
                $imageStore = GreenPlacesHelpers::saveUploadedImage($request->file('image'), config("greenplaces.path.doc.credibilitybrands_image"), "", $isCreateThumb="1", $height=null, $width=700, $request->get('cropped_image'));            
    ...
    public function update(CredibilityBrandRequest $request, $id)
    {                         
        $input = $request->except(['_token', 'image']);           
        try {
            ...
            if ($result) {                                          
                if ($request->file('image', false)) {        
                    $imageStore = GreenPlacesHelpers::saveUploadedImage($request->file('image'), config("greenplaces.path.doc.credibilitybrands_image"), $result->image, $isCreateThumb="1", $height=null, $width=700, $request->get('cropped_image'));            
                    if (isset($imageStore['error_msg']) && $imageStore['error_msg'] == '' && isset($imageStore['name']) && !empty($imageStore['name'])) {
                        $input["image"] = $imageStore['name'];                    
                    }                                
                } 
